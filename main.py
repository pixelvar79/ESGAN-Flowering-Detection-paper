...
try:
    ...
    def train(g_model, d_model, c_model, gan_model, dataset, latent_dim, n_epochs=100):
    ...
    for j in range(3):
        for sample1,n_batchs in zip(percents,batchs):
            ...
            dataset = load_real_samples(X111,yy)
            print(f'training on iteration {j}, {sample1} and {n_batchs}')
            # train model
            #train(g_model, d_model, c_model, gan_model, dataset, latent_dim)
            try:
                # Your training or main script logic here
                train(g_model, d_model, c_model, gan_model, dataset, latent_dim)
