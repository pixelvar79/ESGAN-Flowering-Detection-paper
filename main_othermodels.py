...
for i in range(3):
    for nsample in percents:
        print(i)
        x_train1, x_val, y_train1, y_val = subset_split(stats_df, y, nsample)
        
        # Train KNN model
        knn_model = train_knn(x_train1, y_train1, nsample, dir_out, i)
        knn_preds = knn_model.predict(x_val)
        store_roc_results(knn_model, nsample, i, x_val, y_val, dir_out)  # Save ROC results for KNN
        knn_preds_binary = binarize(knn_preds.reshape(-1, 1), threshold=0.5).ravel()  # Convert to binary class labels
        summary_metrics(knn_model, nsample, y_val, knn_preds_binary, dir_out, i)  # Save summary metrics for KNN

        # Train RF model
        rf_model = train_rf(x_train1, y_train1, nsample, dir_out, i)
        rf_preds = rf_model.predict(x_val)
        store_roc_results(rf_model, nsample, i, x_val, y_val, dir_out)  # Save ROC results for RF
        rf_preds_binary = binarize(rf_preds.reshape(-1, 1), threshold=0.5).ravel()  # Convert to binary class labels
        summary_metrics(rf_model, nsample, y_val, rf_preds_binary, dir_out, i)  # Save summary metrics for RF

        x_train1, x_val, y_train1, y_val = subset_split1(X, y, nsample, resize_images=False)
        ...
        store_roc_results(smallcnn_model,nsample, i, x_val, y_val, dir_out)  # Save ROC results for SmallCNN
        smallcnn_preds_binary = binarize(smallcnn_preds.reshape(-1, 1), threshold=0.5).ravel()  # Convert to binary class labels
        summary_metrics(smallcnn_model, nsample, y_val, smallcnn_preds_binary, dir_out, i)  # Save summary metrics for SmallCNN

        # Set the flag before calling train_transf
        x_train1, x_val, y_train1, y_val = subset_split1(X, y, nsample, resize_images=True)
        transf_model = train_transf(x_train1, y_train1, x_val, y_val, nsample, dir_out, i)
        transf_preds = transf_model.predict(x_val)
        store_roc_results(transf_model, nsample,  i, x_val, y_val, dir_out)  # Save ROC results for Transformer
        transf_preds_binary = binarize(transf_preds.reshape(-1, 1), threshold=0.5).ravel()  # Convert to binary class labels
        summary_metrics(transf_model, nsample, y_val, transf_preds_binary, dir_out, i)  # Save summary metrics for Transformer
