from tensorflow.keras.layers import Input, Conv2D, LeakyReLU, MaxPooling2D, Flatten, Dropout, Dense, Activation, Lambda, Reshape, Conv2DTranspose
...
def define_rf(x_train1, y_train1, nsample, RANDOM_STATE=42):
...
def define_knn(x_train1, y_train1, nsample):
    
    model = KNeighborsClassifier(n_neighbors=5)
    model.fit(x_train1, y_train1)
    
    MODEL_NAME = f'{nsample}_KNN.pkl'
...
def define_transf(x_train1, y_train1, x_val, y_val, nsample):
   
    MODEL_NAME = f'{nsample}_TRANSF_learn.h5'
    ...
    history = model.fit(x_train1, y_train1, epochs=EPOCHS, batch_size=SIZE, validation_data=(x_val, y_val), verbose=2, callbacks=[mc, es])
    
    return model, history


# Define and train the Basic CNN model
def define_smallcnn(x_train1, y_train1, x_val, y_val, nsample):
    
    MODEL_NAME = f'{nsample}_SMALLCNN.h5'
    ...
    model = Sequential()
    model.add(Conv2D(32, (3, 3), activation='relu', kernel_initializer='he_uniform', padding='same', input_shape=(56, 56, 3)))
